...
knitr::opts_knit$set(root.dir = path.to.raab)
knitr::opts_chunk$set(echo = FALSE)

raab<-read.csv(paste0(path.to.raab,resident.data))
raab$raab_id<-raab$regionId

pop<-read.csv(paste0(path.to.pop,pop.data))
...
source(paste0(path.to.scripts,"raab_functions.R"))
...
source(paste0(path.to.scripts,"RAAB7_report_setup.R"))
...
source(paste0(path.to.scripts,"SUM_1.R"))
...
source(paste0(path.to.scripts,"ASA_1.R"))
...
source(paste0(path.to.scripts,"ASA_2.R"))
...
source(paste0(path.to.scripts,"SUM_3_SUM_4_RAAB7.R"))
...
source(paste0(path.to.scripts,"ASA_6_RAAB7.R"))
...
source(paste0(path.to.scripts,"SUM_6_RAAB7.R"))
...
source(paste0(path.to.scripts,"PREV_4.R"))
...
source(paste0(path.to.scripts,"NEW_TAB_1_RAAB7.R"))
...
source(paste0(path.to.scripts,"PREV_14_RAAB7.R"))
source(paste0(path.to.scripts,"ASA_7_RAAB7.R"))
...
source(paste0(path.to.scripts,"SUM_11.R"))
...
source(paste0(path.to.scripts,"OUTC_1_OUTC_2_SUM_12_RAAB7.R"))
...
source(paste0(path.to.scripts,"OUTC_6.R"))
...
For RAAB7, refractive error as a cause of vision impairment is defined as PVA less than 6/12 improving to 6/12 with pinhole. Note: These estimates are based on presenting VA, i.e., with correction if available. Participants with corrected refractive error are not identified, only those with under- or uncorrected refractive error.

## Table 23, table 24

```{r new_tab_4a,echo=FALSE}

source(paste0(path.to.scripts,"NEW_TAB_4_RAAB7.R"))
...
For RAAB7, refractive error coverage (REC) is defined as (A + B) / (A + B + C) where:
...
source(paste0(path.to.scripts,"NEW_TAB_5_RAAB7.R"))
...
source(paste0(path.to.scripts,"NEW_TAB_6_RAAB7.R"))
...
CSC is defined as (X + Y) / (X + Y + Z) where:

X = individuals with unilateral pseudo/aphakia (i.e. operated cataract) and operable cataract in the other eye

Y = individuals with bilateral pseudo/aphakia, regardless of post-operative presenting visual acuity
...
source(paste0(path.to.scripts,"SUMMARY_OUT_RAAB7.R"))

write.table(bigboi,file=paste0(outdir,"/summary/web_summary.csv"),row.names=F,col.names=T,sep=",",na="")
