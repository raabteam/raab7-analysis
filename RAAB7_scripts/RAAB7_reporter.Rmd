...
meta_dataset$title<-paste0("Rapid Assessment of Avoidable Blindness Report: ",raab_meta$survey_title)
...
meta_dataset$doi<-paste0("10.17037/RAABREPORT.",formatC(as.numeric(row.names(raab_meta)),width=8,format = "d",flag="0"))
...
meta_report$bibiographicCitation<-paste0(
  meta_report$creator,
  ". (",
  format(Sys.time(), '%Y'),
  "). ",
...
meta_dataset$bibiographicCitation<-paste0(
...
write.table(asa1.nice.names,here("outputs", ID, "summary", "data", "age_and_sex_distribution_of_people_examined_in_the_sample.csv"),row.names=F,col.names=T,sep=",")

names(asa1.nice.names)<-c("Years","n","%","n","%","n","%")

knitr::kable(asa1.nice.names, booktabs=T, linesep = "", caption = "Age and sex distribution of people examined in the sample",align='lcccccc') %>%
  kable_styling(full_width=T,latex_options = "HOLD_position",font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
  row_spec(4,hline_after=T) %>%
...
asis_output("## Near vision impairment
...
## Severity of cataract-related vision impairment
...
In population-based surveys, refractive error is often defined by an improvement in visual acuity with correction to a specific VA threshold (rather than e.g., in dioptres of myopia). The definition of refractive error used in RAAB7 is UCVA worse than 6/12 improving to 6/12 with spectacle correction or pinhole. The prevalence of refractive error by age group and gender is shown in Tables RE1 and RE2.
...
Note: In contrast to cataract, which predominantly affects the population 50 years and older, refractive error presents across the life course. RAAB does not provide any information on the prevalence of refractive error in the population under 50 years of age.
...
newtab5.nice.names$rec_metric[newtab5.nice.names$rec_metric=="rec"]<-"Distance REC"
newtab5.nice.names$rec_metric[newtab5.nice.names$rec_metric=="erec"]<-"Distance eREC"
...
knitr::kable(newtab5.nice.names, booktabs=T, linesep = "", caption = "Adjusted distance refractive error coverage and effective refractive error coverage", row.names=F,align = 'lccccccc') %>%
  kable_styling(full_width=T, latex_options = "HOLD_position",font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
  column_spec(1,'3cm') %>%
...
treemap(dist.re.need,index="need",vSize="total.extrapolated.n",type="index",palette = need_fig_cols,position.legend="right",fontsize.title = 12,title="Adjusted distance refractive error correction need")
...
asis_output("## Near refractive error coverage")
...
newtab7.nice.names$rec_metric[newtab7.nice.names$rec_metric=="rec"]<-"Near REC"
newtab7.nice.names$rec_metric[newtab7.nice.names$rec_metric=="erec"]<-"Near eREC"
...
treemap(near.re.need,index="need",vSize="total.extrapolated.n",type="index",palette = need_fig_cols, position.legend="right", fontsize.title = 12, title="Adjusted near refractive error correction need")
