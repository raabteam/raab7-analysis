...
if (Sys.Date() > embargo_date){

##############################################

source(here("RAAB7_scripts","raab_functions.R"))
API_username_password<-read.csv(here("data","API_username_password.csv"),check.names=F,na.strings=c("","NA"))
username<-API_username_password$username
password<-API_username_password$password
basic_auth<-API_username_password$basic_auth
raabID<-ID
...
row.order<-c("good.oc","borderline.oc","poor.oc")
cat.oc.time.nice.names<-cat.oc.time.nice.names %>% arrange(match(cat.oc.time.nice.names$oc,row.order))
cat.oc.time.nice.names$oc[cat.oc.time.nice.names$oc=="good.oc"]<-"Good (6/12)"
cat.oc.time.nice.names$oc[cat.oc.time.nice.names$oc=="borderline.oc"]<-"Borderline (<6/12 to 6/60)"
cat.oc.time.nice.names$oc[cat.oc.time.nice.names$oc=="poor.oc"]<-"Poor (<6/60)"
...
row.order<-c("borderline.oc","poor.oc")
...
names(causes.PO.VA.nice.names)<-c("","n","%","n","%","n","%","n","%","n","%")

knitr::kable(causes.PO.VA.nice.names, booktabs=T, linesep = "", caption = "Causes of post-operative presenting VA <6/12 in cataract operated eyes with borderline and poor outcomes (CAT14)",align='lcccccccccc') %>%
  kable_styling(latex_options = c("scale_down","HOLD_position"),font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
  row_spec(3,hline_after=T) %>%
  add_header_above(c("" = 1, "Case selection" = 2, "Intraoperative complications" = 2, "PCO" = 2, "Other sequelae" = 2, "Refractive error" = 2),bold=T)

```


Case selection describes pre-existing co-morbidity (e.g. glaucoma or macular disease) and no improvement in VA with cataract surgery. Posterior capsular opacification (PCO) is a common complication (sequelae) that develops in the years after cataract surgery. ‘Other sequelae’ are any other post-operative complications (excluding PCO) that developed after surgery, such as retinal detachment. Refractive error accounts for residual refractive error and is defined by an improvement in UCVA <6/12 or CVA <6/12 to 6/12 with pinhole.  

PCO is reported separately to other longer-term sequelae as both it and residual refractive error are amenable to relatively simple treatment with spectacles and YAG laser capsulotomy respectively. Strategies to address these two causes of post-operative vision loss will improve cataract surgery outcomes in the population. 

Note: In the absence of clinical records, determining the cause of borderline or poor outcomes via participant-reported history will be subject to recall bias, especially in surgeries that took place a long time ago.   
