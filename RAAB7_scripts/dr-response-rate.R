...
dr.response$female.n[dr.response$Exam.Status=="Diabetes status assessed"] <- sum(raab$diabetes.denom[raab$gender=='female']==1, na.rm=T)
dr.response$female.pct[dr.response$Exam.Status=="Diabetes status assessed"] <- sum(raab$diabetes.denom[raab$gender=='female']==1,na.rm=T) / sum(raab$gender=="female", na.rm=T) *100
dr.response$male.n[dr.response$Exam.Status=="Diabetes status assessed"] <- sum(raab$diabetes.denom[raab$gender=='male']==1,na.rm=T)
dr.response$male.pct[dr.response$Exam.Status=="Diabetes status assessed"] <- sum(raab$diabetes.denom[raab$gender=='male']==1,na.rm=T) / sum(raab$gender=="male", na.rm=T) *100
dr.response$total.n[dr.response$Exam.Status=="Diabetes status assessed"] <- sum(raab$diabetes.denom==1,na.rm=T)
dr.response$total.pct[dr.response$Exam.Status=="Diabetes status assessed"] <- sum(raab$diabetes.denom==1, na.rm=T) / sum((raab$gender=="female" | raab$gender=="male"),na.rm=T) *100
...
dr.response$female.pct[dr.response$Exam.Status=="Known or suspected diabetes"] <- sum(raab$diabetes.known.susp[raab$gender=='female']==1,na.rm=T) / sum(raab$diabetes.denom[raab$gender=='female']==1,na.rm=T) *100
...
dr.response$male.pct[dr.response$Exam.Status=="Known or suspected diabetes"] <- sum(raab$diabetes.known.susp[raab$gender=='male']==1,na.rm=T) / sum(raab$diabetes.denom[raab$gender=='male']==1,na.rm=T) *100
dr.response$total.n[dr.response$Exam.Status=="Known or suspected diabetes"] <- sum(raab$diabetes.known.susp==1,na.rm=T)  
dr.response$total.pct[dr.response$Exam.Status=="Known or suspected diabetes"] <- sum(raab$diabetes.known.susp==1,na.rm=T) / sum(raab$diabetes.denom==1,na.rm=T) *100
...
write.csv(dr.response, here('outputs', 'dr.response.csv'), row.names = FALSE)
