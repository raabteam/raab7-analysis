...
pop<-read.csv(here("data","raabs_pop_618_repo.csv"))
...
source("crude-adj-prev-vi_RAAB5.R")
...
source("crude-adj-cum-prev-vi_RAAB5.R")
...
Table 8 compares the principal causes of blindness, severe vision impairment and moderate vision impairment in the person. In RAAB, the principal cause is determined by comparing the main cause in each eye and selecting the more treatable or preventable cause. The distribution of blindness in the sample by cause is visualised below. Table 9 shows what proportion of vision impairment is attributable to treatable, preventable and posterior segment disease. From these tables the priorities for intervention can be determined.
...
This table shows the principal causes of blindness, disaggregated by gender.
...
source("crude-adj-prev-vi-unilat_RAAB5.R")
...
source("adj-csc-ecsc_RAAB5.R")
source("crude-adj-cat-vi-prev_RAAB5.R")
...
knitr::kable(prev14a.nice.names, booktabs=T, linesep = "", caption = "Adjusted Cataract Surgical Coverage at the person level",row.names=F) %>% 
  ...
  row_spec(0,italic=TRUE) %>%
  column_spec(1,width='4cm') %>%
  pack_rows("Operable threshold <6/18",1,2) %>%
  pack_rows("Operable threshold <6/60",3,4) %>%
  pack_rows("Operable threshold <3/60",5,6) %>%
...
source("surg-type-outcome-gender_RAAB5.R")
...
source("surg-place-outcome_RAAB5.R")
...
1.0 = Cannot see 6/18 but can see 6/60 [2]

1.3 = Cannot see 6/60 but can see 3/60 [3]

1.8 = Cannot see 3/60 but can see 1/60 [4]

3 = Light perception [5]

4 = No light perception [6]
...
A = individuals with unilateral operated cataract attaining 6/18 or better post-operative presenting VA in the operated eye, who have vision impairment from any cause in the other eye
