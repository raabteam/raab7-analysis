...
raab<-read.csv(paste0(path.to.raab,"raabs_612.csv"))
raab<-raab[raab$raab_id==ID,]

pop<-read.csv(paste0(path.to.pop,"raabs_pop_612_peek_format.csv"))
...
knitr::kable(sum3a.nice.names, booktabs=T, linesep = "", caption = "Crude prevalence of blindness (PVA <3/60), severe (PVA <6/60), moderate (PVA <6/18) and mild (PVA <6/12) vision impairment") %>% 
  kable_styling(full_width=T,latex_options="HOLD_position",font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
...
knitr::kable(sum3b.nice.names, booktabs=T, linesep = "", caption = "Adjusted prevalence and extrapolated magnitude of blindness (PVA <3/60), severe (PVA <6/60), moderate (PVA <6/18) and mild (PVA <6/12) vision impairment") %>% 
...
knitr::kable(asa6b.nice.names, booktabs=T, linesep = "", caption = "Adjusted cumulative prevalence and extrapolated magnitude of blindness, severe, moderate and mild vision impairment") %>%
  kable_styling(latex_options = c("scale_down","HOLD_position")) %>%
  row_spec(0,italic=TRUE) %>%
  column_spec(c(4,7,10),width='2cm') %>%
  add_header_above(c("VI level" = 1, "Female" = 3, "Male" = 3, "Total" = 3),bold = T)
...
knitr::kable(sum6.nice.names, booktabs=T, linesep = "", caption = " Principal cause of blindness (PVA <3/60), severe (PVA <6/60), moderate (PVA <6/18) and mild (PVA <6/12) vision impairment") %>% 
...
knitr::kable(sum6ag, booktabs=T, linesep = c('', '', '', '\\addlinespace'), caption = " Principal cause of blindness (PVA <3/60), severe (PVA <6/60), moderate (PVA <6/18) and mild (PVA <6/12) vision impairment, by intervention category") %>% 
  kable_styling(full_width=T,latex_options = "HOLD_position") %>%
  row_spec(0,italic=TRUE) %>%
...
source("PREV_14_RAAB6.R")
source("ASA_7_RAAB6.R")
...
knitr::kable(asa7a.nice.names, booktabs=T, linesep = "", caption = "Crude prevalence of cataract with PinVA <3/60, <6/60, <6/18 and <6/12") %>% 
  kable_styling(latex_options = c("scale_down","HOLD_position"),font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
...
knitr::kable(asa7b.nice.names, booktabs=T, linesep = "", caption = "Adjusted prevalence and extrapolated magnitude of cataract with PinVA <3/60, <6/60, <6/18 and <6/12") %>% 
  ...
  pack_rows("Unilateral",5,8) %>%
  add_header_above(c("Operable cataract" = 1, "Female" = 3, "Male" = 3, "Total" = 3),bold=T) %>%
  add_footnote("Unilateral refers to cases where the other eye has normal PVA",notation="symbol")
...
knitr::kable(prev14a.nice.names, booktabs=T, linesep = "", caption = "Adjusted Cataract Surgical Coverage at the person level") %>% 
  ...
  pack_rows("Operable threshold <6/12",1,1) %>%
  pack_rows("Operable threshold <6/18",2,2) %>%
  pack_rows("Operable threshold <6/60",3,3) %>%
  pack_rows("Operable threshold <3/60",4,4) %>%
...
source("OUTC_1_OUTC_2_SUM_12_RAAB6.R")
...
write.table(newtab4a.nice.names,file=paste0(outdir,"/summary/data/t23.csv"),row.names=F,col.names=T,sep=",")
...
write.table(newtab4b.nice.names,file=paste0(outdir,"/summary/data/t24.csv"),row.names=F,col.names=T,sep=",")
...
Note: These estimates are based on presenting VA, i.e., with correction if available. Participants with corrected refractive error are not identifed, only those with under- or uncorreccted refractive error

## Table 25

Refractive error coverage (REC) is defined as (A + B) / (A + B + C) where

A: Number of people surveyed who use spectacles and have a presenting visual acuity of logmar==0.3 or Snellen 6/12 or better

B: Number of people who use spectacles, have a presenting visual acuity of logmar>0.3 or Snellen <6/12 or better and have a pinhole visual acuity of logmar==0.3 or Snellen 6/12 or better.

C: Number of people who do not use spectacles, have a presenting visual acuity of logmar>0.3 or Snellen <6/12 or better and have a pinhole visual acuity of logmar==0.3 or Snellen 6/12 or better.

```{r new_tab_5,echo=FALSE}

source("NEW_TAB_5.R")

newtab5.nice.names<-newtab5[newtab5$rec_metric=="rec",]
...
write.table(newtab5.nice.names,file=paste0(outdir,"/summary/data/t25.csv"),row.names=F,col.names=T,sep=",")

names(newtab5.nice.names) <- c(" ","Adj. %", "95% CI","Adj. %", "95% CI","Adj. %", "95% CI")

knitr::kable(newtab5.nice.names, booktabs=T, linesep = "", caption = "Refractive error coverage", row.names=F) %>% 
  kable_styling(full_width=T, latex_options = "HOLD_position") %>%
  row_spec(0,italic=TRUE) %>%
  add_header_above(c(" " = 1, "Female" = 2, "Male" = 2, "Total" = 2),bold=T)
...
newtab6.nice.names$specs[newtab6.nice.names$specs=="near"]<-"Use near vision spectacles"
newtab6.nice.names$specs[newtab6.nice.names$specs=="distance"]<-"Use distance vision 
spectacles"

write.table(newtab6.nice.names,file=paste0(outdir,"/summary/data/t26.csv"),row.names=F,col.names=T,sep=",")

names(newtab6.nice.names)<-c("","n","%","n","%","n","%")

knitr::kable(newtab6.nice.names, booktabs=T, linesep = "", caption = "Spectacle use among study participants") %>% 
  kable_styling(full_width=T, latex_options = "HOLD_position") %>%
  row_spec(0,italic=TRUE) %>%
  column_spec(1,width='5cm') %>%
  add_header_above(c(" " = 1, "Female" = 2, "Male" = 2, "Total" = 2),bold=T)
...
Mild VI: PVA of logMAR >=0.47 in the better eye

## Unilateral VI

Blindness: PVA of logMAR >=1.8 in one eye and logMAR 0.3 or better in the other eye

Severe VI: PVA of logMAR >=1.3 in one eye and logMAR 0.3 or better in the other eye

Moderate VI: PVA of logMAR >=1.0 in one eye and logMAR 0.3 or better in the other eye

Mild VI: PVA of logMAR >=0.47 in one eye and logMAR 0.3 or better in the other eye

## Cataract-related VI and surgical outcomes

Operable cataract: PinVA at <3/60, <6/60, <6/18 and <6/12 thresholds plus opacity in the lens plus untreated cataract as principal cause 
