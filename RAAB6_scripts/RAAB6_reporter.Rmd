...
data_root   <- Sys.getenv("RAAB_DATA_ROOT",   unset = "data")
output_root <- Sys.getenv("RAAB_OUTPUT_ROOT", unset = "outputs")

options(knitr.table.format = "latex")
knitr::opts_chunk$set(echo = FALSE)

unlink(here(output_root, ID), recursive = TRUE)
dir.create(here(output_root, ID))
outdir <- ID
dir.create(here(output_root, ID, "summary"))
dir.create(here(output_root, ID, "summary", "data"))
dir.create(here(output_root, ID, "raw"))
dir.create(here(output_root, ID, "raw", "data"))

raab_merged <- read.csv(
  file.path(Sys.getenv("RAAB_DATA_ROOT"), "surveys_612_pva.csv"),
  check.names = FALSE,
  na.strings = c("", "NA")
)
...
pop_merged <- read.csv(
  file.path(Sys.getenv("RAAB_DATA_ROOT"), "pop_612_pva.csv"),
  check.names = FALSE,
  na.strings = c("", "NA")
)
popfives <- pop_merged[pop_merged$raab_id == ID, ]

# meta<-read.csv(here("data","historic_meta.csv"),check.names=F,na.strings=c("","NA"))
meta <- read_xlsx(
  file.path(Sys.getenv("RAAB_METADATA_ROOT"), "raab_metadata_log_current.xlsx"),
  sheet = 1,
  na = c("", "NA")
)

vars_to_drop<-c("map_complete",
                "exclude_repo","repo_meta","repo_data", "embargoed", "regionId",
                "ecsc_2025_consent", "vleg_2026_consent",
                "pi_email1", "pi_email2", "pi_contact_form",
                "trainer_email1", "trainer_email2",
                "implementing_org", "facilitating_org", "funder",
                "VISIONATLAS_URL", "url_report", "url_data")
...
meta_report$doi<-paste0("10.17037/RAABREPORT.",formatC(raab_meta$doi_number,width=8,format = "d",flag="0"))
meta_dataset$doi<-paste0("10.17037/RAABDATASET.",formatC(raab_meta$doi_number,width=8,format = "d",flag="0"))
...
write.table(sum6.nice.names,here("outputs", ID, "summary", "data", ".csv"),row.names=F,col.names=T,sep=",")

sum6.for.tree<-sum6.numeric

names(sum6.nice.names) <- c(' ', 'n', '%', 'n', '%','n', '%','n', '%')

knitr::kable(sum6.nice.names, booktabs=T, linesep = "", caption = " Principal cause of blindness, severe, moderate and mild vision impairment (CAUSE1)", align='lcccccc') %>%
  kable_styling(full_width=T,latex_options = "HOLD_position", font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
...
knitr::kable(sum6ag, booktabs=T, linesep = c('', '', '', '\\addlinespace'), caption = " Principal cause of blindness, severe, moderate and mild vision impairment, by intervention category (CAUSE2)",align='lccccccccc') %>%
  kable_styling(full_width=T,latex_options = "HOLD_position",font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
...
source(here("RAAB6_scripts","crude_cat_ops.R"))
...
This table reports the prevalence of people in the sample with one or both eyes operated, irrespective of post-operative presenting or pinhole acuity in the unoperated eye. The counts only include people operated within a formal healthcare setting, ie, they exclude couched eyes, in line with the definition of effective cataract surgical coverage (see table CAT6).  
...
knitr::kable(catcovdenom.a.nice.names, booktabs=T, linesep = "", caption = "Crude unmet need for cataract surgery at pinhole VA thresholds <3/60, <6/60, <6/18 and <6/12 (CAT2)",align='lccccccccc') %>%
  kable_styling(full_width=T, latex_options = "HOLD_position", font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
  column_spec(1,width='3cm') %>%
  column_spec(c(4,7,10),width='1.5cm') %>%
...
source(here("RAAB6_scripts","surg-time-outcome_RAAB6.R"))
...
source(here("RAAB6_scripts","cause_post_operative_VA_RAAB6.R"))
...
## Prevalence of vision impairment due to distance refractive error
...
write.table(newtab4a.nice.names,here("outputs", ID, "summary", "data", "re1.csv"),row.names=F,col.names=T,sep=",")

names(newtab4a.nice.names)<-c("","n","%","95% CI","n","%","95% CI","n","%","95% CI")

knitr::kable(newtab4a.nice.names, booktabs=T, linesep = "", caption = "Crude prevalence of blindness (PVA <3/60), severe (PVA <6/60), moderate (PVA <6/18) and mild (PVA <6/12) vision impairment due to refractive error (RE1)",align='lccccccccc') %>%
  kable_styling(full_width=T,latex_options="HOLD_position",font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
  add_header_above(c("VI level" = 1, "Female" = 3, "Male" = 3, "Total" = 3),bold=T)
...
write.table(newtab4b.nice.names,here("outputs", ID, "summary", "data", "re2.csv"),row.names=F,col.names=T,sep=",")

names(newtab4b.nice.names)<-c("","%","95% CI","Extraplotated magnitude","%","95% CI","Extrapolated magnitude","%","95% CI","Extrapolated magintude")

knitr::kable(newtab4b.nice.names, booktabs=T, linesep = "", caption = "Adjusted prevalence and extrapolated magnitude of blindness (PVA <3/60), severe (PVA <6/60), moderate (PVA <6/18) and mild (PVA <6/12) vision impairment due to refractive error (RE2)",align='lccccccccc') %>%
  kable_styling(latex_options=c("scale_down","HOLD_position"),font_size = 8) %>%
  row_spec(0,italic=TRUE) %>%
  column_spec(c(4,7,10),width='2cm') %>%
  add_header_above(c("VI level" = 1, "Female" = 3, "Male" = 3, "Total" = 3),bold=T)
...
knitr::kable(newtab5.nice.names, booktabs=T, linesep = "", caption = "Adjusted distance refractive error coverage and effective refractive error coverage (RE4)", row.names=F,align='lcccccc') %>%
  kable_styling(full_width=T, latex_options = "HOLD_position", font_size=8) %>%
  row_spec(0,italic=TRUE) %>%
  add_header_above(c(" " = 1, "Female" = 2, "Male" = 2, "Total" = 2),bold=T) %>%
...
The response rate for the diabetic retinopathy (DR) module can be lower than the standard RAAB survey if participants do not consent to a random blood sugar test or to a dilated eye examination (see Table DR2). Those who do not consent to dilated examination may differ from those who do. For example, they may be younger (e.g., working age rather than retired) or drivers, with the appropriate level of vision required to hold a driving licence.")
...
source(here("RAAB6_scripts","wgq-dis-prev.R"))
...
source(here("RAAB6_scripts","wgq-vi-prev.R"))
...
source(here("RAAB6_scripts","wgq-ecsc.R"))
...
source(here("RAAB6_scripts","equity_blindness_age_RAAB6.R"))
source(here("RAAB6_scripts","equity_ecsc_age_RAAB6.R"))
