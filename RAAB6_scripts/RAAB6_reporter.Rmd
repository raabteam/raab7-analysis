...
raab<-read.csv(here("data","raabs_612.csv"))
raab<-raab[raab$raab_id==ID,]
raab$finished<-as.Date(raab$finished)

pop<-read.csv(here("data","raabs_pop_612_repo.csv"))
...
raab_meta<-meta[meta$raab_id==ID,mrm_vars]
PI<-raab_meta$pi_name
TITLE<-raab_meta$survey_title

write.table(raab_meta,file=here("outputs",ID,"/raw/meta.csv"),row.names=F,col.names=T,sep=",",na="")
write.table(raab,file=here("outputs",ID,"/raw/data/survey_data.csv"),row.names=F,col.names=T,sep=",")
write.table(popfives,file=here("outputs",ID,"/raw/data/population.csv"),row.names=F,col.names=T,sep=",")
...
Survey completed: `r format(max(raab$finished), '%d %B %Y')` 
...
write.table(asa1.nice.names,file=here("outputs",ID,"/summary/data/t2.csv"),row.names=F,col.names=T,sep=",")
...
write.table(asa2.nice.names,file=here("outputs",ID,"/summary/data/t3.csv"),row.names=F,col.names=T,sep=",")
...
write.table(sum3a.nice.names,file=here("outputs",ID,"/summary/data/t4.csv"),row.names=F,col.names=T,sep=",")
...
write.table(sum3b.nice.names,file=here("outputs",ID,"/summary/data/t5.csv"),row.names=F,col.names=T,sep=",")
...
write.table(asa6a.nice.names,file=here("outputs",ID,"/summary/data/t6.csv"),row.names=F,col.names=T,sep=",")
...
write.table(asa6b.nice.names,file=here("outputs",ID,"/summary/data/t7.csv"),row.names=F,col.names=T,sep=",")
...
write.table(sum6ag,file=here("outputs",ID,"/summary/data/t9.csv"),row.names=F,col.names=T,sep=",")
...
write.table(prev4.nice.names,file=here("outputs",ID,"/summary/data/t10.csv"),row.names=F,col.names=T,sep=",")
...
write.table(prev4ag,file=here("outputs",ID,"/summary/data/t11.csv"),row.names=F,col.names=T,sep=",")
...
write.table(unta.nice.names,file=here("outputs",ID,"/summary/data/t12.csv"),row.names=F,col.names=T,sep=",")
...
write.table(untb.nice.names,file=here("outputs",ID,"/summary/data/t13.csv"),row.names=F,col.names=T,sep=",")
...
write.table(asa7a.nice.names,file=here("outputs",ID,"/summary/data/t14.csv"),row.names=F,col.names=T,sep=",")
...
write.table(asa7b.nice.names,file=here("outputs",ID,"/summary/data/t15.csv"),row.names=F,col.names=T,sep=",")
...
prev14.nice.names$num.thresh<-recode_factor(prev14.nice.names$num.thresh,'csc' = "CSC",'ecsc_612' = "eCSC (outcome <6/12)")
...
write.table(prev14a.nice.names,file=here("outputs",ID,"/summary/data/t16.csv"),row.names=F,col.names=T,sep=",")

names(prev14a.nice.names)<-c("Post-operative PVA","Adj. %","95% CI","Adj. %","95% CI","Adj. %","95% CI")

knitr::kable(prev14a.nice.names, booktabs=T, linesep = "", caption = "Adjusted Cataract Surgical Coverage at the person level",row.names=F) %>% 
  kable_styling(latex_options = c("scale_down","HOLD_position")) %>%
  row_spec(0,italic=TRUE) %>%
  column_spec(1,width='4cm') %>%
  pack_rows("Operable threshold <6/12",1,2) %>%
  pack_rows("Operable threshold <6/18",3,4) %>%
  pack_rows("Operable threshold <6/60",5,6) %>%
  pack_rows("Operable threshold <3/60",7,8) %>%
...
write.table(sum11.nice.names,file=here("outputs",ID,"/summary/data/t17.csv"),row.names=F,col.names=T,sep=",")
...
write.table(newtab2.nice.names,file=here("outputs",ID,"/summary/data/t18.csv"),row.names=F,col.names=T,sep=",")
...
write.table(newtab3a.nice.names,file=here("outputs",ID,"/summary/data/t19.csv"),row.names=F,col.names=T,sep=",")
...
write.table(newtab3b.nice.names,file=here("outputs",ID,"/summary/data/t20.csv"),row.names=F,col.names=T,sep=",")
...
write.table(outc6a.nice.names,file=here("outputs",ID,"/summary/data/t21.csv"),row.names=F,col.names=T,sep=",")
...
write.table(outc6b.nice.names,file=here("outputs",ID,"/summary/data/t22.csv"),row.names=F,col.names=T,sep=",")
...
For RAAB6, refractive error as a cause of vision impairment is defined as PVA less than 6/12 improving to 6/12 with pinhole. Note: These estimates are based on presenting VA, i.e., with correction if available. Participants with corrected refractive error are not identified, only those with under- or uncorrected refractive error.
...
write.table(newtab4a.nice.names,file=here("outputs",ID,"/summary/data/t23.csv"),row.names=F,col.names=T,sep=",")
...
write.table(newtab4b.nice.names,file=here("outputs",ID,"/summary/data/t24.csv"),row.names=F,col.names=T,sep=",")
...
For RAAB6, refractive error coverage (REC) is defined as (A + B) / (A + B + C) where:

A = Number of people who use spectacles and have a better eye PVA of 6/12 (met need)

B = Number of people who use spectacles, have a better eye PVA <6/12 and a better eye PinVA of 6/12 (undermet need)

C = Number of people who do not use spectacles, have a better eye PVA <6/12 and a better eye PinVA of 6/12 (unmet need)
...
write.table(newtab5.nice.names,file=here("outputs",ID,"/summary/data/t25.csv"),row.names=F,col.names=T,sep=",")
...
write.table(newtab6.nice.names,file=here("outputs",ID,"/summary/data/t26.csv"),row.names=F,col.names=T,sep=",")
...
Operable cataract: PinVA at <3/60, <6/60 and <6/18 thresholds plus lens opacity plus untreated cataract as principal cause
...
CSC is calculated at the person level, not by eyes, and calculated at various thresholds of operable cataract (see above).
...
A = individuals with unilateral operated cataract attaining 6/12 or better post-operative presenting VA in the operated eye, who have vision impairment from any cause in the other eye

B = individuals with bilateral operated cataract attaining 6/12 or better post-operative presenting VA in at least one eye

X = individuals with unilateral pseudo/aphakia (i.e. operated cataract, excluding couched eyes) and vision impairment in the other eye

Y = individuals with bilateral pseudo/aphakia (i.e. operated cataract, excluding couched eyes), regardless of post-operative presenting visual acuity

Z = individuals with bilateral operable cataract

For further information see: Ramke J, Gilbert CE, Lee AC, Ackland P, Limburg H, Foster A (2017) Effective cataract surgical coverage: An indicator for measuring quality-of-care in the context of Universal Health Coverage. PLoS ONE 12(3): e0172342. doi:10.1371/journal.pone.0172342
...
Note: This is an 'alternative' definition of REC, used where uncorrected VA measurements are not available. It makes an assumption that all participants wearing spectacles for PVA measurement are vision-impaired without them and may slightly over-estimate coverage compared to the 'gold-standard' calculation described here: McCormick I, Mactaggart I, Bastawrous A, Burton MJ, Ramke J. Effective refractive error coverage: an eye health indicator to measure progress towards universal health coverage. Ophthalmic Physiol Opt. 2020;40(1):1-5. doi:10.1111/opo.12662
...
write.table(bigboi,file=here("outputs",ID,"/summary/web_summary.csv"),row.names=F,col.names=T,sep=",",na="")
